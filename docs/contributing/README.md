# Contributing to Dossier

Developer documentation for contributors to the Dossier project.

## Quick Start

1. Read the [Contributing Guidelines](../../CONTRIBUTING.md)
2. Set up your [development environment](development-setup.md) *(coming soon)*
3. Understand our [workflows](workflows.md)
4. Review [issue submission guide](issue-guide.md)
5. Learn about [pull request process](pr-guide.md)

## Development Documentation

### Workflows & Processes
- [GitHub Actions Workflows](workflows.md) - CI/CD automation
- [Issue Submission Guide](issue-guide.md) - How to report issues effectively
- [Pull Request Guide](pr-guide.md) - PR templates and review process
- [Worktrees](worktrees.md) - Using git worktrees for development

### Specialized Topics
- [MCP Server Development](mcp/) - Model Context Protocol integration docs

## Getting Help

- **General questions**: [GitHub Discussions](https://github.com/imboard-ai/ai-dossier/discussions)
- **Bug reports**: [GitHub Issues](https://github.com/imboard-ai/ai-dossier/issues)
- **Security issues**: security@imboard.ai

## Code of Conduct

All contributors must follow our [Code of Conduct](../../CODE_OF_CONDUCT.md).

## License

By contributing to Dossier, you agree that your contributions will be licensed under the [GNU Affero General Public License v3.0 (AGPL-3.0)](../LICENSE).
