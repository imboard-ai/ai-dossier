...
RFC-0001 (`rfcs/0001-batch-cycles.md`; on branch `docs/batch-cycles-rfc` at the time of this run) Step 1 requires `dossier-sched` to demonstrate parity with fleet-cycle on real workloads before any batching begins — specifically to prove the named fleet failure is gone: **slots sitting idle while runnable work exists**. Exit criterion (§G):
