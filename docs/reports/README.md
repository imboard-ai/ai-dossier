...
| [`issue-538-classifier-cost-methodology.md`](./issue-538-classifier-cost-methodology.md) | #538 AC3 cost methodology | PARTIAL — pre-screen hit rate measured on the 15-issue fixture set (7/15 rejected, 0 tokens); post-#538 per-issue cost is still an estimate — live re-measurement pending an ops dispatch run | 2026-09-01 |

Raw evidence backing these reports (baseline tables, scheduler event journals, captured corrupt-state files) lives in [`evidence/`](./evidence/).
