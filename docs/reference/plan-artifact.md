...
| Command | Behavior |
|---|---|
| `ai-dossier plan post --issue <n> --file <md>` | Validates the five sections, stamps `head=` (or takes `--head <sha>`, 7-40 lowercase hex — validated), comments the artifact. `--dry-run` prints the body; `--json` prints `{posted: false, dryRun: true, head, body}` (dry-run) or `{posted: true, head, url}`; refuses a body over 60000 characters pre-flight. `post` never reads the issue's comment thread — only the file it is given — so it prints a disclosure to that effect on **stderr** (leaving `--json` stdout machine-parseable) every run, dry-run and real, rather than leaving `validate`'s `valid: true` to imply the plan reflects discussion it never saw. |
...
| `discussion` | warn | N > 0 issue comments predate the plan (posted before it) — the plan may not reflect them. `plan:v1`/`runstate:v1` artifact comments never count, but only when their opening marker is **well-formed**: a body that merely starts with `<!-- plan:v1 ` and does not parse is discussion, so a comment cannot hide itself from this check. Names the count and the newest predating comment's timestamp. A comment that ties the plan comment's own timestamp exactly counts as neither predating nor postdating. |
| `discussion` | info | N > 0 issue comments postdate the plan (posted after it) — genuine drift since, the direct analogue of `head-distance`. Same artifact-exclusion rule as the `warn` row. Names the count and the newest postdating comment's timestamp. |
| `discussion` | warn | The plan comment's OWN `createdAt` is missing or unreadable, so no comment could be compared against it. Reported (naming how many comments went uncompared) rather than rendering as "this issue has no discussion" — the one conclusion that would certainly be wrong. |
| `discussion` | warn | N > 0 issue comments carry a `createdAt` that could not be parsed and were placed in neither bucket — counted here rather than silently dropped. |
