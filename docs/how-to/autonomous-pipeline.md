...
| Record a blocked batch's own MERGED PR by hand (resolves `pr-detect-ambiguous`, #824; verified same-repo/head/base, never closes the anchor) | `ai-dossier sched attach-pr --batch b1 4270` |
| Requeue a parked batch member (evicted / handed back, #810) — full-cycle from its member branch | `ai-dossier sched requeue --issue 42` |
