...
3. Align the PLANNING template's section names with the artifact's five canonical sections ("Files to Modify" → "Predicted Files", "Test Strategy" → "Test Scope") so the planning file itself is a postable artifact body — `plan post` checks presence of the five sections, not exclusivity; extra sections ride along (the "rich" artifact per C.6). Cross-dossier ripple: implement-issue's Step 1 extract list cites the old names ("files to modify", "test strategy") — aligned registry-side in implement-issue 1.7.3 with this change; setup-issue-workflow's old scaffold carried `## Files to Modify` but its published 1.14.1 already dropped it (the stale in-repo snapshot is the weekly refresh job's domain).
...
## Risk Areas
- **Publishing blocked from this sandbox**: no team signing key available locally and `ai-dossier whoami` = "Not logged in" (login needs a human TTY OAuth flow); sibling publishing hosts unreachable from this session. The edit/lint/dry-runs are local; sign+publish requires hand-off (exact commands) or credentials before the run can complete. Main risk to autonomous completion — hand off per the Guiding Principle at that step. (Resolved mid-run: credentials provided; publish completed.)
- Shadow CLI copies: the globally installed `ai-dossier` is 0.14.0 (npm registry lags; 0.15/0.16 stranded by the sched E404). All `plan` dry-runs must use the worktree build (`node cli/dist/cli.js`). The dossier itself gets a CLI-version note: if `plan` is missing, degrade to fresh planning, never block on tooling.
...
- Section rename ripple: implement-issue's Step 1 cites the old names — fixed in implement-issue 1.7.3 (published; its in-repo snapshot refreshed with this change). setup-issue-workflow's stale in-repo snapshot (1.9.3) still carries `## Files to Modify` in its scaffold, but its published 1.14.1 already dropped it; the weekly refresh job owns that snapshot. Remaining risk: low.
