 ...
 * `prState`/`setupInfo`, #544 `issueLabels`, #596 `openPrForBranch`), and
 * every hand-rolled fake in
...
export function stubGroundTruth(overrides: Partial<GroundTruth> = {}): GroundTruth {
  return {
    ...
    branchHead: () => null,
    prState: () => undefined,
    // `null` = verifiably no open PR, NOT `undefined` (unreachable) — an
    // inert default must never put `enterRecovery` on its degraded path.
