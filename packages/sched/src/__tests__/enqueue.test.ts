...
describe('enqueueEntries', () => {
  ...
  it('applies documented defaults (mode full, tier mid, no deps)', () => {
    const state = enqueueEntries(createEmptyState(NOW), [{ issue: 7 }], NOW);
    expect(state.entries[0]).toMatchObject({ mode: 'full', tier: 'mid', deps: [] });
  });

  it('rejects slot mode without a batch and full mode with one', () => {
    expect(() => enqueueEntries(createEmptyState(NOW), [{ issue: 1, mode: 'slot' }], NOW)).toThrow(
      EnqueueError
    );
    expect(() =>
      enqueueEntries(createEmptyState(NOW), [{ issue: 1, mode: 'full', batch: 'b1' }], NOW)
    ).toThrow(EnqueueError);
  });

  it('rejects self-dependencies and duplicate issues', () => {
    expect(() => enqueueEntries(createEmptyState(NOW), [{ issue: 1, deps: [1] }], NOW)).toThrow(
      /depend on itself/
    );
    expect(() => enqueueEntries(createEmptyState(NOW), [{ issue: 1 }, { issue: 1 }], NOW)).toThrow(
      /Duplicate/
    );
    expect(() => enqueueEntries(createEmptyState(NOW), [{ issue: 0 }], NOW)).toThrow(
      /positive integer/
    );
  });
  ...
  it('rejects dependency cycles across new and existing entries', () => {
    ...
    const fresh = createEmptyState(NOW);
    expect(() =>
      enqueueEntries(
        fresh,
        [
...
describe('parseManifest', () => {
  ...
  it('manifest and flags inputs enqueue to identical entries (AC1 parity)', () => {
    ...
    const viaFlags = enqueueEntries(createEmptyState(NOW), [{ issue: 1 }], NOW);
