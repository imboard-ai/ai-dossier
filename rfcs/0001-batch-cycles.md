...
failure edges (any state):
  in-work/committed → evicted(reason) → requeued{full}   [misclassified, test-failure, revert-conflict]
    [amended #810] evicted(reason) | handed-back(reason) are PARKED — requeued{full} only by an operator (`sched requeue`); a member's own hand-back never counts toward the dissolve threshold
...
failure edges:
  ...
  evictions > ⅓ OR revert-conflict → dissolving → members requeued (smaller batches / full)
    [amended #810] a dissolve never requeues or discards VALIDATED members — on the executing rail it is suppressed (the batch continues), a `full` dissolve blocks for the operator instead
...
| F.6 | **Dependency discovered mid-implementation** | Member posts `blocked reason=dependency-discovered dep=#M` (existing hand-off shape); scheduler: evict without fix attempt, add edge, requeue into a later batch or full-cycle. Batch continues. **Amended #810:** any member-posted `blocked` is a hand-back — the member is parked `handed-back` for the operator (edge and later-batch placement are the operator's `sched requeue` / re-enqueue), and it does not count toward F.8's threshold. |
| F.7 | **Issue larger/riskier than classified** (Q12) | Tripwires: plan-validate floor-scan (before any code — cheapest exit) and implement-time (touched files > 2× estimate, or any floor path). Member stops, posts `blocked reason=misclassified`, scheduler reverts any partial commit, requeues as full-cycle carrying the refined plan. Misclassification is a **cheap, expected event** — this loop is also the classifier's training signal (record predicted vs actual on every issue). **Amended #810:** the member is parked `handed-back` with its plan artifact on the issue; nothing requeues it until an operator decides. |
| F.8 | **Partial batch success** (Q13) | The default outcome, not an exception: batch ships whatever survived; evicted members requeue individually; nothing green is discarded. A dissolved batch loses only uncommitted in-progress work (bounded by F.5's issue-boundary pushes). **Amended #810:** evicted members are parked, not requeued; hand-backs do not count toward the dissolve threshold; a dissolve never requeues or discards validated members. |
