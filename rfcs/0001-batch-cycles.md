...
- **External:** auto-merge watcher Action (imboard repo; known-deficient: gates on absence-of-failure, not presence-of-checks — the #3799 hole's repo-side fix is unshipped).
...
**Modified:** setup-issue-workflow, review-issue, ship-issue, report-issue, gate-issue (batch/slot modes — registry publishes); auto-merge watcher (rebase-merge for `batch-epic` PRs + presence-of-checks fix) — *external repo, long lead, start first*; full-cycle's plan-issue (optional plan-artifact consumption).
