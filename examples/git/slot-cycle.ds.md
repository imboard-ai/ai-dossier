---dossier
{
  ...
  "checksum": {
    "algorithm": "sha256",
    "hash": "847b812ced56d3467847ed3471706ef700d322fc3a9e5c538ff48728b8bcd88f"
  }
}
---
...
Assert every precondition the scheduler owes you, in order — any failure posts `--phase plan --status blocked --kv reason=<slug>` (keys: `mode=slot batch=<batch> reason=<slug>`) and hands back to the scheduler without touching any file:
...
2. **Batch branch checked out** — `git branch --show-current` is the scheduler's batch branch (`batch/<id>-<date>` or whatever the scheduler named — it must NOT be a per-issue branch or the base branch). Failure: `reason=not-batch-branch`.
...
6. **Classify record available** — `ai-dossier runstate last --issue <issue_number> --json` shows `phase=classify` with `mode=slot` (the expected last milestone at dispatch time). Failure: `reason=no-classify-record`.
...
- **error reasons** (`artifact`, `sections`, `missing-file`, `git`) → **refine incrementally, never recreate wholesale**: fetch the artifact (`ai-dossier plan get --issue <issue_number> --json`), edit ONLY the failing parts (fix stale predicted paths, fill missing sections from the issue text — keep every section that passed), and supersede with `ai-dossier plan post --issue <issue_number> --file refined.md` (posting is append-only; readers take the last plan:v1 comment). Re-validate. A plan that cannot be made valid by refinement (issue text itself incoherent) → post `--phase plan --status blocked --kv reason=unrefinable-plan` and hand back.
...
- scope surprise: the artifact's predicted-files count or estimated diff exceeds **2×** the classify record's `est_files`/`est_diff`.
...
**Plan milestone** — save the artifact locally for Step 2 (excluded from the member commit — `.git/info/exclude` of THIS worktree, so the shared batch branch never sees it), then post:
...
One `ac<n>=` line per AC, verbatim and quoted — the same convention plan-issue uses on the full-cycle line, so conformance (Step 3) and every resume tool read a slot trail the standard way. `visual_review=false` always: an issue needing visual review floors to full-cycle by rule (E.2).
...
2. **Implement-time tripwire** (second half of F.7): if touched files exceed **2×** the classify record's `est_files`, or any touched path is a floor area, or the change wants a new package/workspace or deploy-pipeline edit → stop, post `--phase implement --status blocked --kv reason=misclassified --kv mode=slot --kv batch=<batch>` (run id unchanged), hand back. Nothing is committed — the scheduler resets and requeues.
3. **Dependency discovered mid-implementation** (F.6): the work reveals it depends on an unmerged issue `#M` → post `--phase implement --status blocked --kv reason=dependency-discovered --kv dep=<M> --kv mode=slot --kv batch=<batch>` and hand back. The scheduler evicts without a fix attempt and the batch continues — do not attempt the dependency yourself.
4. **Lint (changed files only)** — prefer the fast path:
   ...
   Outcomes: `ok` → done; `task-failed` → fix the reported issues and re-run; `automation-broken`/`capability-unavailable` → do not trust the machinery, fall back to reasoning: detect the toolchain (biome.json / eslint+prettier configs / ruff) and run its fixer on the CHANGED FILES ONLY (`npx biome check --write <files>`, `npx eslint --fix <files> && npx prettier --write <files>`, `ruff check --fix <files> && ruff format <files>`).
...
Dispatch ONE conformance agent on the strongest available model. Same contract as review-issue Agent 7 — inputs, verdict grammar, blindness — with the diff scoped to THIS member's changes:
...
- Any `not-met` → ONE bounded fix loop scoped to that AC (fix, re-run changed-file lint + focused tests), then re-run the conformance agent ONLY. A second `not-met` on the same AC → post `--phase review --status blocked --kv reason=spec-not-met --kv mode=slot --kv batch=<batch>` and hand back (per-issue conformance is the trust anchor; it is never waived — the scheduler requeues the member as full-cycle).
- `unverifiable` → add the test the agent named, then mark the AC met.
...
3. Land exactly one commit, conventional-commit type from the issue labels (`bug`→`fix`, `feature`→`feat`, `docs`→`docs`, `chore`→`chore`, `test`→`test`; default `feat`), title from the issue title minus any leading `<type>:` prefix, issue number as the trailer — following the repo's existing history:

   ```bash
   git commit -m "<type>: <title> (#<issue_number>)"
   ```

   No CI-skip markers, no wip prefixes — the format is exact; the `(#N)` trailer is what the scheduler's failure attribution and eviction machinery key on.
4. Push the batch branch:
   ...
   A rejected push (non-fast-forward) means another writer touched the branch — do NOT force-push; post `--phase review --status blocked --kv reason=push-rejected --kv mode=slot --kv batch=<batch>` (revert the local commit with `git reset --soft $BOUNDARY` first so the worktree is clean for recovery) and hand back.
...
- `blocked_reason`: `misclassified` | `dependency-discovered` | `test-failures` | `spec-not-met` | `unrefinable-plan` | `no-plan-artifact` | `no-classify-record` | `worktree-missing` | `not-batch-branch` | `env-cold` | `dirty-worktree` | `push-rejected` — only on a hand-back
...
- [ ] Misclassification tripwire evaluated at BOTH points: plan-time (floor-area paths, >2× predicted scope) and implement-time (touched files >2× est, floor paths)
- [ ] Lint, typecheck, focused tests ran changed-file-scoped, via `cap run` fast path when available with the reasoning fallback (four-outcome routing followed)
- [ ] Conformance ran blind on the strongest tier; every `met` carries file:line; one bounded fix loop max per AC; a second `not-met` posted `blocked reason=spec-not-met`
...
| Dirty worktree at Step 0 | Previous member crashed mid-issue — post `blocked reason=dirty-worktree`; NEVER clean or reset it yourself, the scheduler owns recovery |
| Push rejected (non-fast-forward) | Another writer touched the batch branch — `git reset --soft $BOUNDARY`, post `blocked reason=push-rejected`; never force-push a shared batch branch |
| Conformance verdict cites files outside this member's diff | Re-run the agent with the diff explicitly scoped to `git diff $BOUNDARY` — a member is never accountable for other members' changes |
