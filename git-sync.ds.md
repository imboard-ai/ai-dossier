...
- **Never auto-merge**, and never auto-rebase a diverged or behind branch — EXCEPT the narrow "safe auto-rebase" case defined under Branch and remote state. Outside that case, report state and recommend an action; let the user decide.
...
| Local diverged from origin (ahead **and** behind), **safe auto-rebase** — ALL of: (a) `git merge-tree --write-tree origin/<branch> HEAD` exits 0 with no conflicts; (b) every local-only commit touches only documentation/report files (`*.md`, `*.txt`, `*.csv`, `*.json`/`*.yaml` under `reports/`, `marketing/`, `docs/`, or similar data/report directories — no source code, no CI config, no lockfiles, no sensitive paths); (c) no local-only commit is a merge commit; (d) working tree is clean after the untracked/modified classification steps ran | `git pull --rebase` then push silently; list both in the silent action plan and final report. If the rebase stops for any reason, `git rebase --abort` and fall back to report-only |
| Local diverged from origin (ahead **and** behind), any other case | Report counts; suggest `git pull --rebase` or merge; do not act |
| Local behind origin, no local commits | Silent `git pull --ff-only` (it cannot lose anything); if ff fails, fall back to report-only |
...
5. **Execute silent actions** in this order: gitignore appends → deletes → stage+commit → safe auto-rebase / ff-only pull → push. Stop on any unexpected error and surface it.
...
- **Untracked file identical to the same path on origin** — e.g. a report a cron box already committed — blocks `git pull` with "untracked working tree file would be overwritten". Treat it as a silent action: remove the untracked copy (content is preserved on origin) and let the pull restore it as tracked. Only do this when `git show origin/<branch>:<path> | diff - <path>` is empty.
...
- [ ] Any auto-rebase performed in this run was conflict-free and touched docs/report files only (recorded in the final report)
